...
存在しないレースコードに対する `DataNotFoundError` は keiba-data-interface の例外です。一括取得に未対応の `DataInterface` で1レースずつ取得する際は、これを欠番として読み飛ばします。それ以外の取得エラー（一括取得経路で送出されたものを含む）は伝播します。
